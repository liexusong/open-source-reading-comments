...
bool is_packet_needed(const char *packet)
{
    ...
    if(ip_header->protocol != IPPROTO_TCP){ // 如果不是TCP协议
        return is_needed;
    }

    size_ip = ip_header->ihl << 2; // IP层数据size
    tot_len = ntohs(ip_header->tot_len); // 包的总长度
    if (size_ip < 20) {
        log_info(LOG_WARN, "Invalid IP header length: %d", size_ip);
        return is_needed;
    }

    tcp_header = (struct tcphdr*)((char *)ip_header + size_ip); // TCP头部(跳过IP头部)
    size_tcp = tcp_header->doff << 2; // TCP包的大小
    if (size_tcp < 20) {
