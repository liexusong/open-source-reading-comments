...
lwan_init(lwan_t *l)
{
     ...
     * respected. */
    lwan_job_thread_init();  // 初始化任务线程(in file lwan-job.c)
    lwan_response_init();    // 初始化回复上下文(in file lwan-response.c)
    lwan_tables_init();

    lwan_module_init(l);     // 初始化模块表
    // 注册一些模块
    ...
    if (!l->config.n_threads) {
        l->thread.count = get_number_of_cpus(); // CPU核数
        if (l->thread.count == 1)
            l->thread.count = 2;
    } else {
    ...
    rlim_t max_open_files = setup_open_file_count_limits();  // 获取操作系统最大能够打开的文件句柄
    allocate_connections(l, (size_t)max_open_files);         // 根据最大能够打开的文件句柄来申请客户端连接对象

    // 每个worker线程可以处理的最大客户端数
    ...
    lwan_thread_init(l);  // 初始化worker线程
    lwan_socket_init(l);  // 初始化监听socket
