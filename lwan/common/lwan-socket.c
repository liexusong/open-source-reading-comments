...
{
    char *node, *port;
    char *listener = strdupa(l->config.listener);  // IP:PORT
    ...
    int ret = getaddrinfo(node, port, &hints, &addrs);  // 获取可用的IP列表
    if (ret)
        lwan_status_critical("getaddrinfo: %s", gai_strerror(ret));

    int fd = bind_and_listen_addrinfos(addrs, l->config.reuse_port);  // 绑定IP
    freeaddrinfo(addrs);
    return fd;
}
